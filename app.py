...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def createTable(self):
        ...
        new_table = NewTable(database_name)
        if new_table.exec_():
            try:
        ...
        if len(selected) == 2:
            ...
            if import_table.exec_() == QtGui.QDialog.Accepted:
                try:
                    ...
                    dialogs.MsgBoxWarning("Warning", "Could not import data from '%s' in table table '%s' from database '%s'" % (import_table.import_table_file, table_name, database_name))
                    traceback.print_exc()
        else:
            dialogs.MsgBoxWarning("Warning", "No table selected!")
    ...
    def exportTable(self):
        ...
        if len(selected) == 2:
            ...
            if export_table.exec_() == QtGui.QDialog.Accepted:
                export_type = ""
                try:
                    if export_table.combo_box.currentIndex() == 0:
                        export_type = "plain"
                        exp_imp.export_plain(database_name, table_name)
                    elif export_table.combo_box.currentIndex() == 1:
                        export_type = "xml"
                        exp_imp.export_xml(database_name, table_name)
                    elif export_table.combo_box.currentIndex() == 2:
                        export_type = "csv"

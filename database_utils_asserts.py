...
def create_table(db_name, table_name, args_list):
    ...
    assert os.path.exists(table_location)
...
def insert_in_table(db_name, table_name, args_list):
    ...
    linii = select_in_table(db_name,table_name, [])
    ...
    assert len(linii) < len(select_in_table(db_name,table_name,[]))
...
def change_database(old_db_name, new_db_name):
    ...
    assert os.path.exists(new_db_location)
    for table in os.listdir(old_db_location):
        ...
        assert os.path.exists(new_table_path)

    shutil.rmtree(old_db_location)
    assert not os.path.exists(old_db_location)
...
def change_table_name(db_name, old_table_name,  new_table_name):
    ...
    assert not os.path.exists(new_table_schema_path)
    assert os.path.exists(table_schema_path)
...
def change_fields_in_table(db_name, table_name, args_list): # args_list = [(old_f_name, new_f_name), ()]
    ...
    if len(args_list) < 1:
        raise IndexError
    oschema = get_schema(db_name,table_name)
    ...
    oschema2 = get_schema(db_name, table_name)
    assert oschema != oschema2
...
def add_column_to_table(db_name, table_name, columns):
    ...
    len_oschema = len(get_schema(db_name, table_name))
    ...
    alta = len(get_schema(db_name, table_name))

    assert len_oschema < alta
...
def drop_column_in_table(db_name, table_name, column_name):
    ...
    len_oschema2 = len(get_schema(db_name, table_name))

    assert len_oschema2 < len_oschema
...
def convert(types_list, field_value_list):
    for i in range(0, len(types_list)):
        if types_list[i] == 'str':
            ...
            assert isinstance(field_value_list[i], str)
            continue
        elif types_list[i] == 'int':
            if field_value_list[i] == '':
                field_value_list[i] = 0
            else:
                field_value_list[i] = int(field_value_list[i])
                assert isinstance(field_value_list[i], int)
            continue
        elif types_list[i] == 'float':
            if field_value_list[i] == '':
                field_value_list[i] = 0.0
            else:
                field_value_list[i] = float(field_value_list[i])
                assert isinstance(field_value_list[i], float)
            continue
    assert isinstance(field_value_list, list)
...
def delete_in_table(db_name, table_name, where_list):
    ...
    for i in range(0, len(temp_linii)):
        temp_linii[i] = convert(types, temp_linii[i])

    copie_linii = []
    for i in range(0, len(temp_linii)):
        copie_linii.append(temp_linii[i])
        ...
        for i in range(0, len(indexes)):
            for j in range(0, len(where_list)):
                if where(temp_linie[indexes[i]], where_list[j][1], where_list[j][2]):
                    ...
                    check = 0
        if check == 1:
            for temp_copie in copie_linii:
                if temp_copie == temp_linie:
                    copie_linii.remove(temp_copie)
    nr_linii = select_in_table(db_name, table_name, [])
    os.remove(table_path)
    assert not os.path.exists(table_path)
    with open(table_path, 'w+') as table:
    ...
    nr_linii2 = select_in_table(db_name, table_name, [])
    assert nr_linii2 < nr_linii
...
def select_in_table(db_name, table_name, select_list, where_list=[]):
    ...
    assert isinstance(select_return, list)
...
def update_in_table(db_name, table_name, update_list, where_list=[]):
    ...
    continut = select_in_table(db_name, table_name, [])
    ...
    with open(table_path, 'w+') as table:
        for linie in temp_linii:
            ...
            table.write('\n')
    assert os.path.exists(table_path)
    continut2 = select_in_table(db_name, table_name, [])

    assert continut != continut2



#create_database('dada')
# #delete_database('ceva')
# insert_in_table('bazadate','tabela', [('field1','ceva')])
# change_fields_in_table('bazadate', 'tabela', [( 'camp5','field10')])
convert(['str'],[2])



# print(select_in_table('bazadate','tabela',['camp2', 'camp3','camp0']))
# delete_in_table('bazadate', 'tabela', [('camp5', '==', 5)])
# print(select_in_table('bazadate','tabela',['camp2', 'camp3','camp0']))
# insert_in_table('bazadate', 'tabela',[('camp3',200), ('camp5', 5),('camp7', 200.0)])
# insert_in_table('bazadate', 'tabela',[('camp3',200), ('camp5', 5),('camp7', 102.2)])
# insert_in_table('bazadate', 'tabela',[('camp3',200), ('camp5', 4),('camp7', 102.2)])
# insert_in_table('bazadate', 'tabela',[('camp3',200), ('camp5', 5),('camp7', 102.2)])
# print(select_in_table('bazadate','tabela',['camp2', 'camp3','camp0','camp7'], [('camp3', '>', 200), ('camp5','==', 5)]))
